...
def main():
    ...
    # print(*periodic_table_list, sep = "\n")

    # Get a chemical formula for a molecule from the user.
    user_formula = input(print("Enter a (molecular) formula: "))

    # Get the mass of a chemical sample in grams from the user.
    gram_mass = input(print("Enter the mass in grams: "))
    ...
    parse_formula(user_formula, gram_mass)

    # Call the compute_molar_mass function to compute the
    # molar mass of the molecule from the compound list.
    total_mass =compute_molar_mass()

    # Compute the number of moles in the sample.
    moles = total_mass / gram_mass
    # Print the molar mass.
    print(f"Molar Mass: {total_mass}")
...
def make_periodic_table():
        ...
        # print(f"{symbol} {name} {Atomic_mass}")
    #return symbol, name, Atomic_mass
    periodic_table_list= {
    ...
    # symbol = periodic_table_list[0]
    # name = periodic_table_list[1]
    # Atomic_mass = periodic_table_list[2]
    # element = periodic_table_list[0, 1, 2]
    # print(element)
    # line_len = len(periodic_table_list.readline())
    # for line_len in periodic_table_list:
    #     iterations = symbol + ' ' + name + ' ' + Atomic_mass
    #     print(iterations)
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for symbol_list in symbol_quantity_list:
        atom = symbol_list[0]
        quantity = symbol_list[1]
        if atom == periodic_table_dict[NAME_INDEX]:
            molar = periodic_table_dict[ATOMIC_MASS_INDEX]
        molar = periodic_table_dict[atom] #not atom[1]?
        molar = molar * quantity
        total_mass =+ molar
    ...
    # not this: mass = atomic * quantity
        # Add the product into the total mass.
    # not this either: total_mass =+ mass
